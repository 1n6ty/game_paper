...
def simulate_expected_score(seed, simulation_time_seconds=60):
    ...
    total_frames = int(simulation_time_seconds * FPS)
...
def init(perpetual: dict, tmp: dict) -> tuple[dict, dict, dict]:
    ...
    """
    print("====== ps milkyFly: init func START ======")
    ...
    print("====== ps milkyFly: init func END ======")
...
def finish(perpetual: dict, tmp: dict, game_data: dict) -> tuple[dict, int]:
    ...
    и пересчитывает максимально возможное количество препятствий, 
    которое могло быть сгенерировано, используя seed из perpetual.
    
    Если полученный счет превышает ожидаемый, то игра считается нечестной (возвращается 0),
    иначе возвращается фактический счет.
    """
    print("====== ps milkyFly: finish func START ======")
    print("Game data:", game_data)

    try:
        reported_score = int(game_data.get("score", 0))
    except ValueError as e:
        print("Reported score must be an integer!")
        print("Game data:", game_data)
        return (perpetual, 0)

    try:
        spentTime = float(game_data.get("spentTime", 0))
    except ValueError as e:
        print("Spent time must be a float!")
        print("Game data:", game_data)
        return (perpetual, 0)

    seed = perpetual.get("last_seed", "")
    if not seed:
        print("Wrong seed:", seed)
        return (perpetual, 0)

    # Симулируем ожидаемый счет за, например, 60 секунд игры (это параметр, который можно настроить)
    # TODO поменять способ верификации результата
    expected_score = simulate_expected_score(seed, simulation_time_seconds=spentTime)
    ...
    if reported_score > expected_score:
        print("Reported score is bigger than Expected score! User is cheater!")
        print("Reported score:", reported_score)
        print("Expected score:", expected_score)
        return (perpetual, 0)
    else:
        ...
        print("====== ps milkyFly: finish func END ======")
