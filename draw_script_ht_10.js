...
class GameEngine {
  ...
  handleClick(e) {
    ...
    if (
      pos.x >= glassX &&
      pos.x <= glassX + GLASS_WIDTH &&
      pos.y >= glassY &&
      pos.y <= glassY + GLASS_HEIGHT
    ) {
      console.log("Стакан нажат");
      if (!this.glassClicked && this.targetFillLevel < TOTAL_DAYS) {
        this.glassClicked = true;
  ...
  gameLoop() {
    ...
    if (this.timerSeconds < 0) {
      this.timerSeconds = 0;
      // Если нужно, здесь можно вызвать метод handleGameOver() или другую логику
    }
  ...
  drawTimer() {
    ...
    this.roundRect(this.ctx, timerX, timerY, timerW, timerH, 10);
    this.ctx.fill();

    const timeStr = this.formatTime(this.timerSeconds);
    this.ctx.fillStyle = "#779FBD";
    this.ctx.font = "600 14px Roboto";
    this.ctx.textAlign = "center";
    this.ctx.textBaseline = "middle";
    this.ctx.fillText(
      `до следующего наполнения осталось ${timeStr}`,
      timerX + timerW / 2,
      timerY + timerH / 2
    );
  }
